        ...
        *,
        ...
        while plan.status.value == "running":
            if pause_event is not None and pause_event.is_set():
                self._plan_executor.pause(plan)
                break
        ...
        if plan.status.value == "completed":
        ...
        elif plan.status.value == "paused":
            plan.metadata["execution_outcome"] = "paused"
            plan.metadata["execution_usage"] = context.usage.to_dict()
            self._save_snapshot(
                plan,
            ...
            await active_observer.on_plan_paused(plan)
