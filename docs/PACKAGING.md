...
and the `css/` and `js/` directories, every file in `shell.WEB_ASSETS`) is
bundled below `_internal/app/ui/nova/web` and reports it as `nova_assets`. The packaged desktop opens Nova (Microsoft Edge
